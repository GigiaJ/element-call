...
export function createCallViewModel$(
    ...
    if (!(userId && deviceId))
        throw new UnknownCallError(
            new Error("userId and deviceId are required"),
        );
                        ...
                        delayId$: scope.behavior(
                            (
                                fromEvent(
                                    ...
                                    [string | undefined, IMembershipManager]
                                >
                ...
                (
                    ...
                    createWrappedUserMedia(scope, {
                        ...
                        handRaised$: scope.behavior(
                            handsRaised$.pipe(
                                map((v) => v[mediaId]?.time ?? null),
                            ),
                        ),
                        reaction$: scope.behavior(
                            reactions$.pipe(
                                map((v) => v[mediaId] ?? undefined),
                            ),
                        ),
                    }),
            ),
        ...
        userMedia$.pipe(
            switchMap((userMedia) => {
                const localUserMedia = userMedia.find(
                    (
                        m,
                    ): m is WrappedUserMediaViewModel &
                        LocalUserMediaViewModel => m.type === "user" && m.local,
                );
    ...
    const windowMode$ = scope.behavior<WindowMode>(
        pipEnabled$.pipe(
            switchMap((pip) =>
                pip ? of<WindowMode>("pip") : naturalWindowMode$,
            ),
        ),
    );
        ...
        switchMap(([userMedia, screenShares]) => {
            ...
            if (userMedia.length <= 2 && screenShares.length === 0) {
                const local = userMedia.find(
                    (
                        vm,
                    ): vm is WrappedUserMediaViewModel &
            ...
            map((l) => {
                switch (l.type) {
                    ...
                    case "spotlight-portrait":
                        ...
                        return l.spotlight.some(
                            (m) => m.type === "screen share",
                        );
            ...
            switchMap((edgeToEdge) => {
                ...
                return windowMode$.pipe(
                    switchMap((mode) => {
                        ...
                        const timeout$ =
                            mode === "flat" ? timer(showFooterMs) : NEVER;

                        return merge(
                            screenTap$.pipe(map(() => "tap screen" as const)),
                            controlsTap$.pipe(
                                map(() => "tap controls" as const),
                            ),
    ...
    const showFooterUrlParams = !(
        urlParams.header === HeaderStyle.None &&
        urlParams.showControls === false
    );
    const showFooter$ = scope.behavior(
        naturallyShowFooter$.pipe(
            map(
                (naturallyShowFooter) =>
                    naturallyShowFooter && showFooterUrlParams,
            ),
        ),
    );
    ...
    const audioOutputSwitcher$ = scope.behavior<{
        ...
        combineLatest(
            ...
            (available, selected) => {
                const selectionType =
                    selected && available.get(selected.id)?.type;
        ...
        showReactions.value$.pipe(
            ...
            >((acc, latest) => {
                ...
                for (const [sender, reaction] of Object.entries(latest)) {
                    const startX =
                        acc.find((v) => v.sender === sender && v.emoji)
            ...
            (acc, latest) => {
                return {
                    playing: latest.filter(
                        (v) =>
                            acc.playing.includes(v) ||
                            acc.newSounds.includes(v),
                    ),
    ...
    const errors$ = scope.behavior<{
        ...
        localMembership.localMemberState$.pipe(
            map((value) => {
                ...
                if (value instanceof ElementCallError)
    ...
    return {
        ...
        fatalError$: scope.behavior(
            errors$.pipe(
                map((errors) => {
                    logger.debug(
                        "errors$ to compute any fatal errors:",
                        errors,
                    );
                    return (
            ...
            remoteMatrixLivekitMembers$.pipe(
                ...
                tap((v) => {
                    const listForLogs = v
                        .map(
                            (m) =>
                                m.membership$.value.userId +
                                "|" +
                                m.membership$.value.deviceId,
                        )
                        .join(",");
                    logger.debug(
